...
#include "game/server/gamemodes/god.h"
	...
	if(m_VoteCloseTime)
	{
		...
		else
		{
			...
			for(std::set<mod *>::iterator i = modLoaderGetter::modloader_main->modList.begin(); i != modLoaderGetter::modloader_main->modList.end(); i++){
				modVoteAction mva = (*i)->onVoteStart(m_aVoteDescription, m_aVoteCommand, m_VoteCreator, this);
				if(mva.breakVote) mvat.breakVote = true;
				if(mva.forceVote) mvat.forceVote = true;
			}
			if(mvat.breakVote){EndVote();
				return;}
			int Total = 0, Yes = 0, No = 0;
			if(!mvat.forceVote && !mvat.breakVote && m_VoteUpdate)
			{
				// count votes
			...
			if(m_VoteEnforce == VOTE_ENFORCE_YES || (mvat.forceVote && mvat.forceVoteType == FORCEVOTE_YES))
			{
			...
			else if(m_VoteEnforce == VOTE_ENFORCE_NO || time_get() > m_VoteCloseTime || ((mvat.forceVote && mvat.forceVoteType == FORCEVOTE_NO)))
			{
				EndVote();
...
void CGameContext::OnInit(/*class IKernel *pKernel*/)
{
	...
	// select gametype
	if(str_comp(g_Config.m_SvGametype, "smod") == 0)
		m_pController = new CGameControllerSMod(this);
	else if(str_comp(g_Config.m_SvGametype, "ctf") == 0)
		m_pController = new CGameControllerCTF(this);
	else if(str_comp(g_Config.m_SvGametype, "tdm") == 0)
		m_pController = new CGameControllerTDM(this);
	else if(str_comp(g_Config.m_SvGametype, "god") == 0){
		m_pController = new CGameControllerGod(this);
	}
	else
